...
- **Эксплуатационные ограничения**:
  ...
  - Развёртывание: только в облаке (Yandex Cloud).
...
Датасет содержит ~47 млн транзакций на файл (40 файлов, CSV). Структура:

| Поле | Тип | Описание |
|------|-----|----------|
| tranaction_id | long | ID транзакции |
| tx_datetime | string | Дата и время |
| customer_id | long | ID клиента (~1 млн уникальных) |
| terminal_id | int | ID терминала (~2000 уникальных) |
| tx_amount | double | Сумма (медиана ~45₽, макс ~3700₽) |
| tx_time_seconds | long | Время в секундах от эпохи |
| tx_time_days | long | Время в днях от эпохи |
| tx_fraud | int | Метка мошенничества (0/1), ~6.5% fraud |
| tx_fraud_scenario | int | Сценарий (0-3) |

### Обнаруженные проблемы качества данных

1. NULL terminal_id (~85/файл)
2. terminal_id = 'Err' (~2200/файл)
3. Отрицательный customer_id (~96/файл)
4. Некорректное время 24:00:00 (~92/файл)
5. Нулевые суммы tx_amount (~936/файл)
6. Дубликаты tranaction_id (~16/файл)

Очистка автоматизирована: `scripts/data_cleaning.py`, результат в Parquet.
